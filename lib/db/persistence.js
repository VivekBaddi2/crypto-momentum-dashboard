...
export async function loadPairCatalog() {
  const db = await getDatabase();
  return db.collection("pairCatalog").findOne(
    { _id: "binance-usdt-market-cap" },
    { projection: { _id: 0 } }
  );
}

export async function savePairCatalog({ pairs, source }) {
  const db = await getDatabase();
  const updatedAt = new Date();
  await db.collection("pairCatalog").updateOne(
    { _id: "binance-usdt-market-cap" },
    { $set: { pairs, source, updatedAt } },
    { upsert: true }
  );
  return { pairs, source, updatedAt };
}
